			...
			createDateTimePicker = function( input ) {
				...
				calendar
					.on('click.xdsoft','td',function() {
						...
						if($("#lineShow").length>0){
							var selectTime=_xdsoft_datetime.str();
							var nowTime=(_xdsoft_datetime.now()).dateFormat( options.format );
							var selY=selectTime.split(".")[0];
							var selm=selectTime.split(".")[1];
							var selr=selectTime.split(".")[2];
							var noY=nowTime.split(".")[0];
							var nom=nowTime.split(".")[1];
							var nor=nowTime.split(".")[2];
							if(selY>noY||(selY==noY&&selm>nom)||(selY==noY&&selm==nom&&selr>nor)){
								alert("时间不能超过当前时间，请重新选择");
								return false;
							}
						}
						input.val( _xdsoft_datetime.str() );
